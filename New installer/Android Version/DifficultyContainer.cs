...
public partial class DifficultyContainer : PanelContainer
{
	...
	[Export] private Panel Difficulty;
	[Export] private Label PanelTitle;
	...
	{
		PanelTitle.Text = Tr("Difficulty_N");
		...
		Hardmode.TooltipText = Tr("HARDMODE");
		Hardcore.Text = Tr("Hardcore_L");
		Hardcore.TooltipText = Tr("HARDCORE");		
		...
		FilthChallenge.TooltipText = Tr("FILTH");
		...
		PanelTitle.AddThemeFontSizeOverride("font_size", 100);	
		Title.AddThemeFontSizeOverride("font_size", 70);
		Filth.AddThemeFontSizeOverride("font_size", 70);
		Expanded.AddThemeFontSizeOverride("font_size", 70);	
		Extra.AddThemeFontSizeOverride("font_size", 70);	

		if (PanelTitle.GetLineCount() > 2)		
			PanelTitle.AddThemeFontSizeOverride("font_size", 70);					
		
		if (Title.Text.Length > 22)		
			Title.AddThemeFontSizeOverride("font_size", 55);			
		
		if (Expanded.Text.Length > 22)		
			Expanded.AddThemeFontSizeOverride("font_size", 55);
			
		if (Filth.Text.Length > 22)		
			Filth.AddThemeFontSizeOverride("font_size", 55);
		
		if (Extra.Text.Length > 22)
			Extra.AddThemeFontSizeOverride("font_size", 55);

		infoButtons[0].Pressed += () => {VicePatcher.OpenInfoWindowDifficulty(Title.TooltipText, Title.Text, null);};
		infoButtons[1].Pressed += () => {VicePatcher.OpenInfoWindowDifficulty(Expanded.TooltipText, Expanded.Text, null);};
		infoButtons[2].Pressed += () => {VicePatcher.OpenInfoWindowDifficulty(Filth.TooltipText, Filth.Text, null);};
		infoButtons[3].Pressed += () => {VicePatcher.OpenInfoWindowDifficulty(Extra.TooltipText, Extra.Text, null);};
	}
	...
	void SetupButtons()
	{
		...
		allCheckboxes = [Challenge, Hardmode, Hardcore, TrueHardcore, Progression, AllBattles, UltraHardcore, FilthChallenge, RestoreFilth, Rookie, Tanemon, FairBattles, Tournaments, RNG, Mono4K, Mono8K, Agumon];
		for (int i = 4; i < infoButtons.Length; i++)
		{
			int cat = i - 4;
			infoButtons[i].Pressed += () =>
			{
				VicePatcher.OpenInfoWindowDifficulty(allCheckboxes[cat].TooltipText, allCheckboxes[cat].Text, GetDifficultySLink(cat));
			};
		}		
	}
	...
	string GetDifficultySLink(int linkvalue)
	{
		switch(linkvalue)
		{
			case 1:
			case 5:
			return "https://docs.google.com/spreadsheets/d/14wAuaMaLK6YZqwGXQSaG4RKCBfH6JWnrXg2cVAL_KW4/edit?usp=sharing";
			case 2:
			case 3:
			return "https://docs.google.com/spreadsheets/d/13hKiq2UGXikMRRLJfKpkudDk9RZ4BRvlCYma1Sof11A/edit?usp=sharing";
			case 4:
			return "https://docs.google.com/spreadsheets/d/147dr8Uq_LT1X0STYwuvS59PwbZ_l4hK97GsVCvzJISU/edit?usp=sharing";
			case 7:
			return "https://docs.google.com/spreadsheets/d/1I0rPMIKg5Q7H1EwDlyiOBTGRkKlFxaXM_O4I5mxMdfg/edit?usp=sharing";
			default:
			return null;
		}
	}
